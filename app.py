...
Bug fixes applied:
  1. CRITICAL: page() now accepts **kwargs and passes them to render_template_string.
     Previously, EVERY data-driven route (leaderboard, profile, play, daily, etc.)
     silently produced empty/broken output because page() only forwarded 4 base
     variables to render_template_string, leaving grid, rows, season, etc. undefined.
     All routes now call page() directly (no double render_template_string).

  2. random.seed() global-state mutation fixed: create_game_state, select_players_by_fame,
     and build_grid_validated now use a local random.Random instance so concurrent
     gevent greenlets cannot corrupt each other's RNG state.

  3. SQL column-name injection: get_user_rating() and get_user_rank() now validate
     the `col` parameter against a strict whitelist before interpolating into SQL.

  4. FLAG_MAP and TEAM_LOGOS added as Jinja2 env globals so they are always available
     in templates without needing to be forwarded explicitly.

  5. mode parameter in /leaderboard route validated before use.

  6. n_low in select_players_by_fame guaranteed >= 0 via clamped arithmetic.

  7. Nested get_valid_category uses passed-in rng instead of global random.

  8. Contact-form rate-limit counter incremented before response to prevent
     double-submit races.

  9. api_end_game: "daily" mode now also awards XP (was skipped due to early return).

 10. Duplicate cell values in build_grid_validated pad-loop prevented.

 11. play() route: player_type param no longer defined but unused — removed cleanly.

 12. Removed dead outer render_template_string wrappers from all routes.
"""
...
def player_matches_cell(player, cell, ds):
    ...
    if ct == "combo":
        # FIX: strip whitespace around each part when splitting on "+"
...
#  FIX #2: accepts an optional `rng` (random.Random instance) so callers can
#  pass a local seeded RNG without mutating the module-level random state.
# ══════════════════════════════════════════════════════════════════════════════
...
def select_players_by_fame(pool, difficulty, n=25, player_type=None, rng=None):
    ...
    FIX: uses `rng` (a random.Random instance) if provided, otherwise falls back
    to the module-level random — prevents global RNG state corruption.
    """
    if rng is None:
        rng = _random_module  # module-level fallback
    ...
    # FIX: clamp n_low to 0 (rounding can make n_high + n_medium > n)
    n_low    = max(0, n - n_high - n_medium)

    rng.shuffle(high_f)
    rng.shuffle(medium_f)
    rng.shuffle(low_f)
    ...
    log.info(
        f"select_players_by_fame: diff={difficulty} "
...
# ══════════════════════════════════════════════════════════════════════════════
#  GRID BUILDER
#  FIX #2: accepts `rng` to avoid global RNG mutation.
#  FIX #10: pad-loop checks `seen` to prevent duplicate cell values.
# ══════════════════════════════════════════════════════════════════════════════
...
def build_grid_validated(size, ds, difficulty, pool, rng=None):
    ...
    # FIX #10: pad-if-short loop respects `seen` to avoid duplicate values
    while len(cells) < n and valid_teams:
        ...
        if not added:
            break  # no more unique valid teams available
...
def create_game_state(ds, grid_size, difficulty, seed=None, player_type=None):
    ...
    FIX #2: uses a local random.Random instance (seeded when seed is provided)
    instead of seeding the global random module, preventing race conditions in
    concurrent gevent greenlets.
    """
...
# FIX #3: whitelist allowed column names to prevent SQL injection
...
# ═══════════════════════════════════════════════════════════════════════════════
#  FIX #1 — page() now accepts **kwargs and forwards them to render_template_string.
#
#  BEFORE: page() called render_template_string internally with only 4 base vars;
#          every route then called render_template_string(page(...), **vars) which
#          ran Jinja2 a second time on already-rendered HTML, making the extra
#          vars completely invisible to the templates.  Result: grid, rows, season,
#          profile_user, etc. were all undefined (empty) in every data-driven page.
#
#  AFTER:  page() merges the base vars with caller-supplied kwargs and renders once.
#          All routes now call page(..., key=val) directly — no outer wrapper.
# ═══════════════════════════════════════════════════════════════════════════════
...
"""
    # Merge base context with caller-supplied kwargs.
    # Caller kwargs take lower priority than the 4 base vars so they can't
    # accidentally shadow current_user / streak_current etc.
    context = dict(kwargs)
...
# ── HTML body templates (unchanged — see original) ────────────────────────────
# NOTE: These are kept identical to the original; omitted here for brevity.
# In the real file, paste HOME_BODY, GAME_BODY, MATCHMAKING_BODY, ROOM_BODY,
# LEADERBOARD_BODY, PROFILE_BODY, DAILY_BODY, ABOUT_BODY, CONTACT_BODY,
# PRIVACY_BODY, TERMS_BODY exactly as in the original.
# The fixes below (routes) are what matter.

HOME_BODY        = "<!-- paste original HOME_BODY here -->"
GAME_BODY        = "<!-- paste original GAME_BODY here -->"
MATCHMAKING_BODY = "<!-- paste original MATCHMAKING_BODY here -->"
ROOM_BODY        = "<!-- paste original ROOM_BODY here -->"
LEADERBOARD_BODY = "<!-- paste original LEADERBOARD_BODY here -->"
PROFILE_BODY     = "<!-- paste original PROFILE_BODY here -->"
DAILY_BODY       = "<!-- paste original DAILY_BODY here -->"
ABOUT_BODY       = "<!-- paste original ABOUT_BODY here -->"
CONTACT_BODY     = "<!-- paste original CONTACT_BODY here -->"
PRIVACY_BODY     = "<!-- paste original PRIVACY_BODY here -->"
TERMS_BODY       = "<!-- paste original TERMS_BODY here -->"


# ═══════════════════════════════════════════════════════════════════════════════
#  ROUTES — FIX #1 applied: all routes call page(..., **vars) directly.
#           No outer render_template_string wrapper anywhere.
# ═══════════════════════════════════════════════════════════════════════════════

@app.route("/")
def home():
    # FIX: return page(...) directly — no render_template_string wrapper
...
def privacy():
    ...
    # FIX: pass sections directly to page()
...
def play():
    ...
    # FIX #5 / #11: removed unused player_type param from play route
    ...
    # FIX #1: all vars passed to page() directly — single render_template_string call
    return page(
        GAME_BODY, "Play",
        grid             = grid,
        grid_json        = grid_json,
...
def matchmaking():
    ...
    grid_size  = int(request.args.get("grid_size", 3))
    difficulty = request.args.get("difficulty", "normal")
    # FIX #1: pass vars to page() directly
...
def leaderboard():
    ...
    # FIX #5: validate mode to prevent unexpected values reaching SQL column selection
...
# ── API endpoints (largely unchanged; key fixes noted inline) ─────────────────
...
def api_contact():
    ...
    # FIX #8: increment BEFORE returning so double-submits still get counted
...
def api_end_game():
    ...
    # FIX #9: daily mode now also gets streak + XP updates (was missing before)
    ...
    # XP for all non-quit, authenticated game modes with a positive score
...
def on_queue(data):
    ...
    if cands:
        ...
        query_db(
            ...
            (code, opp["user_id"], current_user.id, json.dumps(state, default=str), "rated", "active"),
            commit=True)
...
if __name__ == "__main__":
    ...
    print(f"""
╔══════════════════════════════════════════════════════════╗
║   🏏  Cricket Bingo v7 (Fixed)                           ║
╠══════════════════════════════════════════════════════════╣
║  Fixes:  page(**kwargs), local RNG, SQL whitelist,       ║
║          mode validation, XP for daily, pad dedup,       ║
║          session.modified, contact rate-limit            ║
╚══════════════════════════════════════════════════════════╝
""")
