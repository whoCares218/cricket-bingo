...
def select_players_by_fame(pool, difficulty, n=25, player_type=None):
    ...
    random.shuffle(high_f); random.shuffle(medium_f); random.shuffle(low_f)
    selected  = high_f[:n_high] + medium_f[:n_medium] + low_f[:max(0, n_low)]
    if len(selected) < n:
    ...
    log.info(
        f"select_players_by_fame: player_type={player_type} → {len(selected)} players "
...
def create_game_state(ds, grid_size, difficulty, seed=None, player_type=None):
    ...
    state = {
        ...
        # FIX Bug 2: seed=0 is falsy with `or`; use explicit None check
...
def get_streak_data(uid):
    ...
    return {"current": row["current_streak"], "best": row["best_streak"],
            "freeze": row["freeze_available"], "last_played": row["last_played_date"]}
...
def update_streak(uid):
    ...
    row = query_db("SELECT * FROM daily_streaks WHERE user_id=?", (uid,), one=True)
    if not row:
        query_db("INSERT INTO daily_streaks(user_id,current_streak,best_streak,last_played_date,freeze_available)"
    ...
    if consecutive:
        ...
        query_db("UPDATE daily_streaks SET current_streak=?,best_streak=?,last_played_date=?,freeze_available=?"
                 " WHERE user_id=?", (new_streak, new_best, today, new_freeze, uid), commit=True)
        return {"current": new_streak, "best": new_best,
                "is_new": True, "broken": False, "used_freeze": used_freeze}
    else:
        query_db("UPDATE daily_streaks SET current_streak=1,last_played_date=? WHERE user_id=?",
...
def xp_to_level(xp):
    for i in range(len(XP_THRESHOLDS) - 1, -1, -1):
        if xp >= XP_THRESHOLDS[i]: return min(i + 1, len(LEVEL_NAMES))
    return 1
...
def xp_next_level(xp):
    lvl = xp_to_level(xp)
    if lvl >= len(XP_THRESHOLDS): return 0, 0
...
def get_xp_data(uid):
    ...
    if not row: return {"total": 0, "level": 1, "name": "Rookie"}
    lvl = row["level"]
...
def update_xp(uid, gain):
    ...
    if not row:
        ...
        old_total = row["total_xp"]; old_lvl = row["level"]
        new_total = old_total + gain; new_lvl = xp_to_level(new_total)
        query_db("UPDATE user_xp SET total_xp=?,level=? WHERE user_id=?",
                 (new_total, new_lvl, uid), commit=True)
    return {"total": new_total, "level": new_lvl, "name": level_name(new_lvl),
            "gained": gain, "leveled_up": new_lvl > old_lvl, "old_level": old_lvl}


# ── Rarity ─────────────────────────────────────────────────────────────────────
def track_cell_picks_for_daily(challenge_date, filled_by):
    for idx_str, pname in filled_by.items():
        if not pname or pname.endswith("_wc"): continue
        try: cidx = int(idx_str)
...
def get_rarity_for_cells(challenge_date, n_cells):
    ...
    for cidx in range(n_cells):
        ...
        top_row = query_db(
            ...
            " ORDER BY pick_count DESC LIMIT 5", (challenge_date, cidx))
...
.justify-center{justify-content:center;}.flex-wrap{flex-wrap:wrap;}.text-center{text-align:center;}
...
.step-indicator{display:flex;align-items:center;justify-content:center;gap:0;padding:18px 0;margin-bottom:8px;border-bottom:1px solid var(--bdr);flex-wrap:wrap;gap:8px;}
.step-item{display:flex;align-items:center;gap:8px;}
.step-item+.step-item::before{content:'';width:40px;height:1px;background:var(--bdr2);margin:0 8px;}
.step-num{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.78rem;font-weight:700;font-family:var(--font-head);border:1.5px solid var(--bdr2);color:var(--txt3);flex-shrink:0;transition:all .25s;}
...
::-webkit-scrollbar{width:5px;}::-webkit-scrollbar-track{background:var(--bg);}::-webkit-scrollbar-thumb{background:var(--sur3);border-radius:3px;}
...
.streak-frozen{color:var(--teal);border-color:rgba(46,196,182,.3);background:rgba(46,196,182,.08);}
...
# ── FIX Bug 1: page() now accepts **kwargs and forwards them to render_template_string
# so every route body template (GAME_BODY, LEADERBOARD_BODY, PROFILE_BODY, etc.)
# actually receives its Jinja2 variables.  All routes call page() directly instead
# of wrapping it in a second render_template_string() call.
...
{{GLOBAL_SCRIPTS}}
        ...
        **kwargs,   # forwards all route-specific template variables
    )
...
HOME_BODY = """
<style>
.setup-wrap{max-width:680px;margin:0 auto;padding:40px 28px 88px;display:flex;flex-direction:column;align-items:center;}
.setup-step{width:100%;animation:fade-in .22s ease;}
.step-indicator{justify-content:center;margin-bottom:32px;}
.pool-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;width:100%;margin-bottom:24px;}
.mode-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;width:100%;margin-bottom:24px;}
.sel-card{background:var(--sur);border:1.5px solid var(--bdr);border-radius:var(--r-xl);padding:20px 16px;text-align:center;cursor:pointer;transition:border-color .18s,background .18s,transform .18s,box-shadow .18s;user-select:none;}
.sel-card:hover{border-color:var(--acc);background:var(--acc-dim);transform:translateY(-2px);box-shadow:0 6px 20px var(--acc-glow);}
...
.sel-card-title{font-family:var(--font-head);font-size:.92rem;font-weight:700;color:var(--txt);display:block;margin-bottom:3px;}
.sel-card-sub{font-size:.72rem;color:var(--txt2);display:block;line-height:1.4;}
.sel-card-badge{display:inline-block;margin-top:8px;padding:2px 9px;border-radius:99px;font-size:.62rem;font-weight:700;background:var(--acc-dim);color:var(--acc);border:1px solid rgba(245,166,35,.28);}
.settings-card{background:var(--sur);border:1px solid var(--bdr);border-radius:var(--r-xl);padding:24px 20px;width:100%;margin-bottom:24px;}
.settings-row{display:grid;grid-template-columns:1fr 1fr;gap:16px;}
.step-section-label{font-size:.7rem;font-weight:700;color:var(--acc);text-transform:uppercase;letter-spacing:.1em;margin-bottom:16px;font-family:var(--font-head);display:flex;align-items:center;gap:10px;}
.step-section-label::after{content:'';flex:1;height:1px;background:var(--bdr);}
.fame-strip{display:flex;align-items:center;flex-wrap:wrap;gap:7px;margin-top:10px;padding:9px 14px;background:var(--sur2);border-radius:var(--r-lg);border:1px solid var(--bdr);font-size:.72rem;color:var(--txt3);}
.fame-strip .fp{padding:2px 9px;border-radius:99px;font-weight:700;font-size:.68rem;}
.action-row{display:flex;gap:12px;justify-content:center;width:100%;}
@media(max-width:520px){.pool-grid{grid-template-columns:1fr;}.mode-grid{grid-template-columns:1fr;}.settings-row{grid-template-columns:1fr;}.setup-wrap{padding:24px 14px 60px;}}
</style>
  ...
  <div id="s1" class="setup-step">
    ...
    <div class="pool-grid">
      ...
      <div class="sel-card" id="pool-ipl26" onclick="selectPool('ipl26',this)">
        ...
        <span class="sel-card-sub">Current season squads only</span>
  ...
  <div id="s2" class="setup-step" style="display:none;">
    ...
    <div class="mode-grid">
      ...
      <div class="sel-card" id="mode-friends" onclick="selectModeAndAdvance('friends',this)">
        ...
        <span class="sel-card-sub">Share a 6-digit room code</span>
...
function startGame() {
  ...
  if(selMode === 'rated')
    ...
    // FIX Bug 3: removed dead `const loginBtn` variable that was here
...
<div class="container page" style="max-width:820px;">
  ...
  <div class="grid-3 gap-3 mb-4">
    <div class="stat-card"><div class="stat-label">Score</div><div class="stat-value" style="color:var(--acc);" id="sc">0</div></div>
    <div class="stat-card"><div class="stat-label">Remaining</div><div class="stat-value" id="pl">{{ total_players }}</div></div>
    <div class="stat-card"><div class="stat-label">Accuracy</div><div class="stat-value" style="color:var(--blue);" id="ac">—</div></div>
  </div>
  ...
  <div class="flex gap-3 mt-5 justify-center flex-wrap">
    ...
    <button class="sound-btn" id="snd-btn" onclick="toggleSound()" title="Toggle sounds">🔊</button>
  </div>
</div>

<div id="emod" class="modal-overlay" style="display:none;">
  <div class="modal text-center" style="max-width:480px;">
    ...
    <div id="streak-block" style="display:none;margin:12px 0;padding:12px 16px;background:rgba(245,166,35,.08);border:1px solid rgba(245,166,35,.2);border-radius:var(--r-lg);">
      <div id="streak-content" style="font-family:'Outfit',sans-serif;font-size:1.1rem;font-weight:800;"></div>
      <div id="streak-sub" style="font-size:.75rem;color:var(--txt2);margin-top:3px;"></div>
    </div>

    <div id="xp-block" style="display:none;margin:10px 0;padding:10px 14px;background:rgba(155,114,247,.08);border:1px solid rgba(155,114,247,.2);border-radius:var(--r-lg);">
      <div id="xp-content" style="font-family:'Outfit',sans-serif;font-size:1rem;font-weight:700;color:var(--pur);"></div>
      <div id="xp-bar-wrap" class="xp-bar-wrap" style="margin-top:7px;"><div id="xp-bar" class="xp-bar" style="width:0%;"></div></div>
      <div id="xp-sub" style="font-size:.7rem;color:var(--txt3);margin-top:4px;"></div>
    </div>
    ...
    <div id="share-block" style="display:none;">
      <div class="share-grid-wrap" id="share-grid-text"></div>
      <div class="share-btn-row">
        <button class="btn btn-secondary btn-sm" onclick="copyShareGrid()">📋 Copy</button>
        <button class="btn btn-secondary btn-sm" onclick="shareWhatsApp()">💬 WhatsApp</button>
        <button class="btn btn-secondary btn-sm" onclick="shareTwitter()">🐦 Tweet</button>
      </div>
    </div>

    <button onclick="toggleSolutions()" class="btn btn-outline btn-sm w-full mb-3 mt-3" id="sol-btn">🔍 View All Answers</button>
    <div id="solutions-panel" style="display:none;text-align:left;margin-bottom:16px;">
      <div id="solutions-content" style="max-height:260px;overflow-y:auto;"></div>
    </div>
    ...
    <div id="guest-nudge" style="display:none;margin-top:14px;padding:12px 14px;background:rgba(79,142,247,.08);border:1px solid rgba(79,142,247,.25);border-radius:var(--r-lg);text-align:center;">
      <div style="font-size:.82rem;color:var(--txt2);margin-bottom:8px;">🔑 Sign in to track your streak, earn XP, and appear on the leaderboard</div>
      <a href="/login/google" class="btn btn-primary btn-sm w-full">Sign In with Google</a>
    </div>
  </div>
</div>

<div id="levelup-overlay" class="levelup-overlay" style="display:none;" onclick="this.style.display='none'">
  <div class="levelup-card">
    <div style="font-size:3rem;margin-bottom:8px;">🎖️</div>
    <div style="font-family:'Outfit',sans-serif;font-size:1.5rem;font-weight:800;color:var(--pur);margin-bottom:4px;">Level Up!</div>
    <div id="levelup-name" style="font-size:1rem;color:var(--txt2);margin-bottom:12px;"></div>
    <div style="font-size:.78rem;color:var(--txt3);">Tap to continue</div>
  </div>
</div>
...
const G = {
  ...
  idx: 0, gstate: new Array({{ grid_size * grid_size }}).fill(null),
  filled_by: new Array({{ grid_size * grid_size }}).fill(null),
  correct: 0, wrong: 0, skips: 0, wcUsed: false, wcPenalty: 0,
  t0: Date.now(), tsec: 10, tleft: 10, tint: null,
  ended: false, clickable: false, rarityData: null,
};
...
function playCorrect(){ playTone(523,'sine',.12,.3); setTimeout(()=>playTone(659,'sine',.12,.3),80); setTimeout(()=>playTone(784,'sine',.2,.3),160); }
function playWrong(){ playTone(200,'sawtooth',.18,.2); setTimeout(()=>playTone(150,'sawtooth',.25,.3),120); }
function playComplete(){ [523,659,784,1047].forEach((f,i)=>setTimeout(()=>playTone(f,'sine',.22,.35),i*100)); }
function playStreak(){ [784,880,1047,1319].forEach((f,i)=>setTimeout(()=>playTone(f,'sine',.18,.4),i*80)); }
...
if(G.room){
  ...
  sock.on('opponent_move', d => {
    ...
    if(el){ el.textContent = d.score; el.classList.remove('pulse'); void el.offsetWidth; el.classList.add('pulse'); }
  });
}

function calcScore(){
  const filled = G.gstate.every(x => x !== null);
  return Math.max(0, G.correct*100 - G.wrong*40 - G.wcPenalty + (filled ? 200 : 0));
}
...
function showP(){
  if(G.ended) return;
  const pn = document.getElementById('pn'), ps = document.getElementById('ps');
  if(!pn || !ps) return;
  if(!G.players || G.players.length === 0){ pn.textContent='⚠ No players loaded'; ps.textContent='Check '+G.ds+'.json'; return; }
...
function startTimer(){
  clearInterval(G.tint); G.tleft = G.tsec; G.clickable = true; tickTimer();
  G.tint = setInterval(()=>{ G.tleft--; tickTimer(); if(G.tleft<=0){ clearInterval(G.tint); timeUp(); } }, 1000);
}
...
function timeUp(){ G.wrong++; G.idx++; playWrong(); refresh(); toast('⏰ Time up! −40','warn'); setTimeout(showP, 200); }
...
function clickCell(i){
  ...
  .then(r=>r.json()).then(res=>{
    ...
    if(res.correct){
      G.correct++; G.gstate[i] = name; G.filled_by[i] = name;
      el.classList.add('filled'); playCorrect(); toast('✅ Correct!','success');
    } else {
      G.wrong++; el.classList.add('wrong'); setTimeout(()=>el.classList.remove('wrong'),500);
      playWrong(); toast('❌ Wrong!','error');
    }
    G.idx++;
...
function doSkip(){
  if(G.ended) return;
  G.skips++; G.idx++; clearInterval(G.tint); refresh();
...
function doWildcard(){
  ...
  .then(r=>r.json()).then(d=>{
    if(d.matching_cells && d.matching_cells.length > 0){
      ...
      d.matching_cells.forEach((ci, idx) => {
        if(G.gstate[ci] === null){
          setTimeout(()=>{
            ...
            if(el){ el.classList.remove('wildcard-hint'); el.classList.add('wc-filled'); G.gstate[ci] = name; G.filled_by[ci] = name+' (WC)'; }
          }, idx*120);
          filled++;
        }
      });
      ...
      setTimeout(()=>{
        if(G.room) sock.emit('player_move',{room:G.room,score:calcScore()});
        refresh(); toast('🃏 '+filled+' cell(s) auto-filled!','info');
        if(G.gstate.every(x=>x!==null)){ end('grid_complete'); return; }
        setTimeout(showP, d.matching_cells.length*120+300);
      }, d.matching_cells.length*120+100);
    } else { toast('🃏 No valid cells found','warn'); }
  });
}

function quitGame(){ if(G.ended) return; end('quit'); }

// FIX Bug 4: removed visibilitychange handler that fired on every tab-switch,
// ending the game with score=0/reason=quit whenever the user briefly left the tab.
// The beforeunload handler below is sufficient for actual page navigation/close.
window.addEventListener('beforeunload', function(){
  if(!G.ended){
    const payload = JSON.stringify({
      ...
      accuracy:0, reason:'quit', filled_by:{}
    });
...
function rarityTier(pct){ if(pct===null||pct===undefined) return 'common'; if(pct<1) return 'legendary'; if(pct<5) return 'epic'; if(pct<15) return 'rare'; return 'common'; }
function rarityEmoji(tier){ return {legendary:'💎',epic:'🔵',rare:'🟢',common:'⚪',empty:'⬜',wrong:'❌'}[tier]||'⬜'; }

let _shareText = '';
function buildShareGrid(rarityData){
  const n = G.gs;
  ...
  for(let r=0;r<n;r++){
    ...
    for(let c=0;c<n;c++){
      const i=r*n+c;
      if(G.gstate[i]===null){ row+='⬜'; continue; }
      if(rarityData&&rarityData[i]){
        const total=rarityData[i].total||0;
        let pct=null;
        if(total>0){ const myPick=G.filled_by[i]; const myEntry=(rarityData[i].top||[]).find(p=>p.name===myPick); if(myEntry) pct=(myEntry.count/total*100); }
        row+=rarityEmoji(rarityTier(pct));
      } else { row+='🟢'; }
    }
    gridRows.push(row);
  }
  const modeLabel={solo:'🎮 Solo',rated:'⚡ Rated',friends:'👥 Friends',daily:'📅 Daily'}[G.mode]||G.mode;
  _shareText=['🏏 Cricket Bingo · '+today,'Score: '+score+' · '+modeLabel,gridRows.join('\n'),'💎 Legendary  🔵 Epic  🟢 Rare  ⚪ Common','cricket-bingo.in'].join('\n');
  return _shareText;
}
function copyShareGrid(){ navigator.clipboard.writeText(_shareText).then(()=>toast('Copied! 📋','success')).catch(()=>toast(_shareText,'info')); }
function shareWhatsApp(){ window.open('https://wa.me/?text='+encodeURIComponent(_shareText),'_blank'); }
function shareTwitter(){ const t=_shareText.split('\n').slice(0,4).join('\n'); window.open('https://twitter.com/intent/tweet?text='+encodeURIComponent(t),'_blank'); }

function toggleSolutions(){
  const panel=document.getElementById('solutions-panel'), btn=document.getElementById('sol-btn');
  if(panel.style.display==='none'){ panel.style.display=''; btn.textContent='🔼 Hide Answers'; renderSolutions(); }
  else { panel.style.display='none'; btn.textContent='🔍 View All Answers'; }
}

function renderSolutions(){
  const cont=document.getElementById('solutions-content');
  const grid={{ grid_json | safe }};
  const rarityData=G.rarityData;
  let html='';
  grid.forEach((cell,i)=>{
    const sols=G.solutions[String(i)]||[];
    const filledBy=G.filled_by[i];
    let rarityHtml='';
    if(rarityData&&rarityData[i]&&rarityData[i].total>0){
      const total=rarityData[i].total;
      const myPick=filledBy?filledBy.replace(' (WC)',''):null;
      const myEntry=myPick?(rarityData[i].top||[]).find(p=>p.name===myPick):null;
      const pct=myEntry?(myEntry.count/total*100):null;
      const tier=rarityTier(pct);
      rarityHtml=pct!==null?`<span class="rarity-chip rarity-${tier}" style="margin-left:6px;">${rarityEmoji(tier)} ${pct<1?'<1':Math.round(pct)}%</span>`:'';
      const topPicks=(rarityData[i].top||[]).slice(0,3).map(p=>`<span style="font-size:.68rem;color:var(--txt3);margin-right:8px;">${p.name} <span style="color:var(--txt3);">${(p.count/total*100)<1?'<1':Math.round(p.count/total*100)}%</span></span>`).join('');
      if(topPicks) rarityHtml+=`<div style="margin-top:4px;">${topPicks}</div>`;
    }
    html+=`<div style="margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid var(--bdr);">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;flex-wrap:wrap;gap:4px;">
        <div style="font-size:.72rem;font-weight:700;color:var(--acc);font-family:'Outfit',sans-serif;">
          ${cell.type==='team'?'🏏':cell.type==='nation'?'🌍':'🏆'} ${cell.value}
          ${filledBy?'<span style="color:var(--green);font-size:.65rem;margin-left:6px;">✓ '+filledBy+'</span>':'<span style="color:var(--txt3);font-size:.65rem;margin-left:4px;">⬜ Unfilled</span>'}
        </div>${rarityHtml}
      </div>
...
function end(reason){
  ...
  const filledByMap={};
  G.filled_by.forEach((name,i)=>{ if(name) filledByMap[String(i)]=name; });
  if(reason==='grid_complete') playComplete();
  fetch('/api/end_game',{
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({room_code:G.room, mode:G.mode, data_source:G.ds, difficulty:G.diff, grid_size:G.gs, score, correct:G.correct, wrong:G.wrong, skips:G.skips, wc_used:G.wcUsed, elapsed, accuracy:acc, reason, filled_by:filledByMap})
  })
  .then(r=>r.json()).then(d=>{
    const done=G.gstate.every(x=>x!==null);
    const isQuit=reason==='quit';
    document.getElementById('ee').textContent=done?'🏆':(isQuit?'🏳':'🎯');
    document.getElementById('et').textContent=done?'Grid Complete!':(isQuit?'Game Quit':'Game Over');
    document.getElementById('es').textContent=isQuit?'0':score;
    const wcNote=G.wcUsed?' · 🃏 −20 WC':'';
    const skipNote=G.skips>0?' · ⏭ '+G.skips+' skip(s)':'';
    document.getElementById('ed').textContent=isQuit?'You quit — full rating penalty applied.':'Acc: '+acc+'% · ✅ '+G.correct+' · ❌ '+G.wrong+wcNote+skipNote+' · '+elapsed+'s';

    if(d.streak&&d.streak.is_new){
      const sb=document.getElementById('streak-block'), sc=document.getElementById('streak-content'), ss=document.getElementById('streak-sub');
      sb.style.display='';
      if(d.streak.broken){ sc.innerHTML='<span style="color:var(--txt2);">💔 Streak reset to 1</span>'; ss.textContent='Your streak was broken. Best: '+d.streak.best+' days'; }
      else {
        const flame=d.streak.current>=30?'🔥🔥🔥':d.streak.current>=7?'🔥🔥':'🔥';
        sc.innerHTML=flame+' <span style="color:var(--acc);">'+d.streak.current+'-day streak!</span>';
        const milestones={7:'🏏 Bronze Bat',30:'🥈 Silver Bat',100:'🥇 Gold Bat',365:'👑 Legend Bat'};
        const nextM=[7,30,100,365].find(m=>m>d.streak.current);
        ss.textContent=nextM?(nextM-d.streak.current)+' days to '+milestones[nextM]:'All-time best: '+d.streak.best+' days';
        if(d.streak.used_freeze) ss.textContent='🧊 Freeze used! '+ss.textContent;
        if([7,30,100,365].includes(d.streak.current)) playStreak();
      }
    }

    if(d.xp){
      const xb=document.getElementById('xp-block'), xc=document.getElementById('xp-content'), xs=document.getElementById('xp-sub'), xbar=document.getElementById('xp-bar');
      xb.style.display='';
      xc.textContent='+'+d.xp.gained+' XP  ·  Level '+d.xp.level+': '+d.xp.name;
      const toNext=d.xp.to_next||0, range=d.xp.range||1;
      const pct=range>0?Math.min(100,((range-toNext)/range*100)):100;
      setTimeout(()=>{ xbar.style.width=pct+'%'; },200);
      xs.textContent=toNext>0?toNext+' XP to next level':'Max level reached!';
      if(d.xp.leveled_up){ setTimeout(()=>{ document.getElementById('levelup-name').textContent='Level '+d.xp.level+': '+d.xp.name; document.getElementById('levelup-overlay').style.display='flex'; playStreak(); },800); }
    }

    const rcEl=document.getElementById('rating-change'), rkEl=document.getElementById('rank-display'), rtEl=document.getElementById('rating-type');
    if(d.rating_change!==undefined&&d.rating_change!==0){
      const rc=Math.round(d.rating_change), isUp=rc>0;
      rcEl.innerHTML=`<span style="font-size:1.8rem;">${isUp?'▲':'▼'}</span> <span style="font-size:1.8rem;font-weight:800;color:${isUp?'var(--green)':'var(--red)'};">${isUp?'+':''}${rc}</span> <span style="font-size:.9rem;color:var(--txt2);">Rating</span>`;
      const modeLabel=G.mode==='rated'?'Multiplayer ELO':G.mode==='friends'?'Friends ELO':'Solo ELO';
      const diffLabel={easy:'🟢 Easy',normal:'🟡 Normal',hard:'🔴 Hard'}[G.diff]||G.diff;
      rtEl.textContent=modeLabel+' · '+diffLabel;
      if(d.par_score&&G.mode==='solo'){ const beat=score>=d.par_score; rkEl.textContent=(beat?'✅ Beat':'❌ Below')+' par ('+d.par_score+')'; if(d.new_rating) rkEl.textContent+=' · Now '+Math.round(d.new_rating)+' ELO'; }
      else { if(d.new_rank) rkEl.textContent='Rank #'+d.new_rank; if(d.new_rating) rkEl.textContent+=(rkEl.textContent?' · ':'')+Math.round(d.new_rating)+' ELO'; }
    } else if(d.new_rating){
      rcEl.innerHTML='<span style="font-size:1.1rem;color:var(--txt2);">No rating change</span>';
      rtEl.textContent=G.mode==='rated'?'Multiplayer ELO':G.mode==='friends'?'Friends ELO':'Solo ELO';
      rkEl.textContent='Current: '+Math.round(d.new_rating)+' ELO';
    }

    G.rarityData=d.rarity||null;
    const shareText=buildShareGrid(G.rarityData);
    if(shareText&&!isQuit){ document.getElementById('share-grid-text').textContent=shareText; document.getElementById('share-block').style.display=''; }
    document.getElementById('emod').style.display='flex';
    if(!d.new_rating&&!d.streak){ const nudge=document.getElementById('guest-nudge'); if(nudge) nudge.style.display=''; }
  })
  .catch(()=>{ document.getElementById('emod').style.display='flex'; });
}

(function initGame(){
  function boot(){ if(!G.players||G.players.length===0){ const pn=document.getElementById('pn'); if(pn) pn.textContent='⚠ No players — check '+G.ds+'.json'; return; } showP(); }
...
<div class="container page">
  ...
  <div class="table-wrap">
    <table>
      <thead><tr><th style="width:48px;">#</th><th>Player</th><th>Tier</th><th>Rating</th><th>W / L</th><th class="hide-sm">Win %</th></tr></thead>
      <tbody>
        {% for r in rows %}
        <tr>
          <td>{% if loop.index==1 %}<span style="font-size:1.1rem;">🥇</span>{% elif loop.index==2 %}<span style="font-size:1.1rem;">🥈</span>{% elif loop.index==3 %}<span style="font-size:1.1rem;">🥉</span>{% else %}<span style="color:var(--txt3);font-size:.83rem;">{{ loop.index }}</span>{% endif %}</td>
        ...
        {% endfor %}
        {% if not rows %}<tr><td colspan="6" style="text-align:center;padding:56px;color:var(--txt3);">No ranked players yet — be the first! 🚀</td></tr>{% endif %}
      </tbody>
    </table>
  </div>
</div>
...
<div class="container page">
    ...
    <div class="flex items-center gap-4 flex-wrap">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {% if xp_data %}<span class="level-badge">Lv {{ xp_data.level }} · {{ xp_data.name }}</span>{% endif %}
          {% if streak_data and streak_data.current > 0 %}
          <span class="streak-badge" title="{{ streak_data.current }}-day daily streak">
            <span class="streak-fire">🔥</span> {{ streak_data.current }}
            {% if streak_data.freeze > 0 %}<span style="margin-left:3px;" title="{{ streak_data.freeze }} freeze(s)">🧊</span>{% endif %}
          </span>
          {% endif %}
        </div>
        {% if xp_data %}
  ...
  <div class="card">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>Result</th><th>Score</th><th class="hide-sm">Opponent</th><th class="hide-sm">Δ Rating</th><th>Mode</th><th class="hide-sm">Difficulty</th><th class="hide-sm">Date</th></tr></thead>
        <tbody>
          {% for m in matches %}
          {% set is_solo = m.mode in ['solo','daily_solo'] %}
          <tr>
            <td>
              {% if is_solo %}{% if m.won %}<span style="color:var(--green);font-weight:700;font-size:.82rem;">✅ DONE</span>{% else %}<span style="color:var(--red);font-weight:700;font-size:.82rem;">🏳 QUIT</span>{% endif %}
            ...
            <td class="hide-sm" style="color:var(--txt2);">{% if is_solo %}<span style="color:var(--pur);font-size:.78rem;">Solo</span>{% else %}{{ m.opponent or '—' }}{% endif %}</td>
            <td class="hide-sm">
            ...
            <td class="hide-sm">
              ...
              <span style="font-size:.72rem;color:{{ diff_colors.get(m.difficulty,'var(--txt3)') }};">{{ {'easy':'🟢 Easy','normal':'🟡 Normal','hard':'🔴 Hard'}.get(m.difficulty, m.difficulty) }}</span>
            </td>
            <td class="hide-sm" style="color:var(--txt3);font-size:.78rem;">{{ m.played_at[:10] if m.played_at else '—' }}</td>
          </tr>
          {% endfor %}
...
function switchTab(t){
  ...
  ['mp','solo'].forEach(x=>{ document.getElementById('tab-'+x).classList.toggle('active', x===t); });
}
</script>
"""
...
<div class="container page">
  <div class="flex justify-between items-center mb-5 flex-wrap gap-3">
    ...
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      ...
      {% else %}
        <span class="badge" style="color:var(--green);border-color:rgba(45,211,111,.3);padding:8px 16px;font-size:.8rem;background:rgba(45,211,111,.07);">✅ Completed Today</span>
      {% endif %}
    </div>
  </div>

  {% if current_user.is_authenticated %}
  <div id="streak-card" style="display:none;margin-bottom:20px;"></div>
  {% endif %}
...
fetch('/api/streak').then(r=>r.json()).then(d=>{
  const s=d.streak, xp=d.xp;
  const card=document.getElementById('streak-card');
  const milestones={7:'🏏 Bronze Bat',30:'🥈 Silver Bat',100:'🥇 Gold Bat',365:'👑 Legend Bat'};
  const nextMilestone=[7,30,100,365].find(m=>m>(s.current||0));
  const flame=(s.current||0)>=30?'🔥🔥🔥':(s.current||0)>=7?'🔥🔥':'🔥';
  card.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
      <div class="stat-card"><div class="stat-label">Current Streak</div><div class="stat-value" style="color:var(--acc);">${flame} ${s.current||0}</div><div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">days</div></div>
      <div class="stat-card"><div class="stat-label">Best Streak</div><div class="stat-value" style="color:var(--pur);">${s.best||0}</div><div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">days</div></div>
      <div class="stat-card"><div class="stat-label">Level</div><div class="stat-value" style="color:var(--blue);font-size:1.6rem;">${xp.level||1}</div><div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">${xp.name||'Rookie'}</div></div>
    </div>
    ${nextMilestone?`<p style="text-align:center;color:var(--txt3);font-size:.78rem;margin-top:10px;">${nextMilestone-(s.current||0)} days to ${milestones[nextMilestone]}${(s.freeze||0)>0?' · 🧊 '+s.freeze+' freeze available':''}</p>`:''}
  `;
  card.style.display='';
}).catch(()=>{});
</script>
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ROUTES
#  FIX Bug 1: every route calls page() directly (no wrapping render_template_string)
#  so route-specific Jinja2 variables actually reach the template.
# ═══════════════════════════════════════════════════════════════════════════════
...
def leaderboard():
    ...
    for r in raw:
        ...
        rows.append({"user_id": r["user_id"], "name": r["name"], "rating": r["rating"],
                     ...
                     "tier": t, "tier_color": tc, "tier_icon": ti, "win_rate": wr})
...
def profile(user_id):
    ...
    return page(PROFILE_BODY, ur["name"],
        ...
        xp_data=xp_d, streak_data=get_streak_data(user_id))
...
def api_validate_move():
    ...
    if correct:
        if len(gstate) != len(grid): gstate = [None] * len(grid)
        gstate[cidx]     = str(pid)
...
def api_wildcard_hint():
    ...
    if len(gstate) != len(grid): gstate = [None] * len(grid)
    for i in cells: gstate[i] = player_name + "_wc"
    gi["grid_state"]      = gstate
    session["game_state"] = gi
...
def api_end_game():
    ...
    if gmode == "daily" and filled_by:
        try: track_cell_picks_for_daily(today, filled_by)
        except Exception as e: log.error(f"Rarity track failed: {e}")

    if gmode == "daily":
        try:
            rarity = get_rarity_for_cells(today, grid_size * grid_size)
            result["rarity"] = rarity
        except Exception as e: log.error(f"Rarity fetch failed: {e}")
    ...
    if gmode == "daily":
        ...
        except Exception as e: log.error(f"Daily result insert failed: {e}")

    elif gmode in ("solo", "daily_solo") and season and is_auth:
        ...
        try:
            query_db("""UPDATE season_ratings
                SET solo_rating=?, solo_games=solo_games+1, total_games=total_games+1,
                    accuracy_sum=accuracy_sum+?, time_sum=time_sum+?
                WHERE user_id=? AND season_id=?""",
                (new_solo, accuracy, elapsed, current_user.id, season["id"]), commit=True)
            query_db("""INSERT INTO matches(player1_id,player2_id,winner_id,player1_score,player1_time,player1_accuracy,rating_change,mode,data_source,grid_size,difficulty,season_id)
                VALUES(?,NULL,?,?,?,?,?,?,?,?,?,?)""",
                (current_user.id,
        ...
        except Exception as e: log.error(f"Solo DB write failed uid={current_user.id}: {e}", exc_info=True)
        result.update({"rating_change": delta_solo, "new_rating": new_solo,
                       "par_score": round(par), "difficulty": difficulty, "quit": quit_game})
        new_rank = get_user_rank(current_user.id, season["id"], "solo_rating")
        if new_rank: result["new_rank"] = new_rank
        log.info(f"SOLO {'QUIT' if quit_game else 'END'} uid={current_user.id} diff={difficulty} k={k} score={score:.0f} par={par:.0f} act={act:.2f} Δ={delta_solo:+.1f} ({old_solo:.0f}→{new_solo:.0f})")
        ...
        if row and row["status"] != "finished":
            ...
            results[str(current_user.id)] = {"score": score, "elapsed": elapsed, "accuracy": accuracy, "quit": data.get("reason") == "quit"}
            ...
            if len(results) >= 2:
                ...
                if q1 and not q2:   winner = p2
                elif q2 and not q1: winner = p1
                elif r1["score"] > r2["score"]: winner = p1
                elif r2["score"] > r1["score"]: winner = p2
                else: winner = p1 if r1.get("elapsed", 9999) <= r2.get("elapsed", 9999) else p2
                ...
                query_db("""INSERT INTO matches(player1_id,player2_id,winner_id,player1_score,player2_score,player1_time,player2_time,player1_accuracy,player2_accuracy,rating_change,mode,data_source,grid_size,difficulty,season_id)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (p1,p2,winner, r1["score"],r2["score"], r1.get("elapsed",0),r2.get("elapsed",0), r1.get("accuracy",0),r2.get("accuracy",0),
                ...
                if new_rank: result["new_rank"] = new_rank
                socketio.emit("game_result", {"rating_change": -my_delta, "winner": winner != current_user.id}, room=room_code)
                log.info(f"MP result p1={p1} p2={p2} winner={winner} diff={mp_diff} k={k_mp} scores={r1['score']:.0f}/{r2['score']:.0f} Δ={delta:+.1f}")
            else:
    ...
    if is_auth and not quit_game and gmode == "daily":
        ...
        except Exception as e: log.error(f"Streak update failed: {e}")
    ...
    if is_auth and not quit_game and score > 0:
        try:
            ...
            result["xp"] = xp_data
        except Exception as e: log.error(f"XP update failed: {e}")
...
if __name__ == "__main__":
    ...
    print(f"""
╔══════════════════════════════════════════════════════════╗
║   🏏  Cricket Bingo v7  — Streak · XP · Rarity · Share  ║
╠══════════════════════════════════════════════════════════╣
