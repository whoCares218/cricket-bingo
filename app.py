...
def update_streak(uid):
    """Update daily streak. Returns dict with new streak info."""
    today     = date.today().isoformat()
    ...
    # Freeze: if missed exactly yesterday but have a freeze, consume it
    used_freeze = False
    if last == yesterday:
        consecutive = True
    ...
    if consecutive:
        ...
        # Award one freeze every 7-day milestone
        if new_streak % 7 == 0:
...
# ══════════════════════════════════════════════════════════════════════════════
#  RARITY TRACKING (daily challenge only — shared seed = comparable picks)
# ══════════════════════════════════════════════════════════════════════════════

def track_cell_picks_for_daily(challenge_date, filled_by):
    """
    filled_by: dict { cell_index(str) -> player_name }
    Upserts pick counts in cell_picks table.
    """
...
def get_rarity_for_cells(challenge_date, n_cells):
    """Returns list of rarity dicts per cell."""
    result = []
...
.streak-badge {
  ...
  font-family: var(--font-head); font-size: .8rem; font-weight: 700; color: var(--acc);
...
/* ── XP BAR ── */
.xp-bar-wrap {
  height: 3px; background: var(--sur3); border-radius: 99px; overflow: hidden; margin-top: 4px;
}
.xp-bar { height: 100%; border-radius: 99px; background: var(--pur); transition: width .6s ease; }
.level-badge {
  display: inline-flex; align-items: center; gap: 5px;
  font-size: .72rem; font-weight: 700; color: var(--pur);
  background: rgba(155,114,247,.12); border: 1px solid rgba(155,114,247,.25);
  border-radius: 99px; padding: 3px 9px; font-family: var(--font-head);
}

/* ── SHARE GRID ── */
.share-grid-wrap {
  background: var(--sur2); border: 1px solid var(--bdr2);
  border-radius: var(--r-lg); padding: 14px 16px; margin: 12px 0;
...
.levelup-card {
  background: var(--sur); border: 2px solid var(--pur);
  border-radius: var(--r-2xl); padding: 32px 40px; text-align: center;
  box-shadow: 0 0 60px rgba(155,114,247,.4);
...
/* ── SOUND TOGGLE ── */
.sound-btn {
  width: 32px; height: 32px; background: var(--sur2); border: 1px solid var(--bdr2);
  border-radius: var(--r-md); display: flex; align-items: center; justify-content: center;
  cursor: pointer; font-size: .9rem; transition: all .18s; flex-shrink: 0;
}
...
<nav class="nav">
  ...
  <div class="nav-actions">
    {% if current_user.is_authenticated %}
      ...
      <span class="streak-badge hide-sm" title="{{ streak_current }}-day streak! Keep playing daily to maintain it.">
...
def page(body, title="Cricket Bingo", extra_head=""):
    nav = NAV_HTML()
    # Inject streak + level data for logged-in users
    streak_current = 0
    user_level = 1
...
function startGame() {
  ...
  if(selMode === 'rated')
    ...
    const loginBtn = document.querySelector('a[href="/login/google"]');
...
const G = {
  ...
  // rarity data from server (daily only)
  rarityData: null,
};
...
function playCorrect(){
  ...
  setTimeout(()=>playTone(659, 'sine', 0.12, 0.3), 80);
  setTimeout(()=>playTone(784, 'sine', 0.2, 0.3), 160);
}
function playWrong(){
  playTone(200, 'sawtooth', 0.18, 0.2);
  setTimeout(()=>playTone(150, 'sawtooth', 0.25, 0.3), 120);
}
function playComplete(){
  [523,659,784,1047].forEach((f,i)=>setTimeout(()=>playTone(f,'sine',0.22,0.35),i*100));
}
function playStreak(){
  [784,880,1047,1319].forEach((f,i)=>setTimeout(()=>playTone(f,'sine',0.18,0.4),i*80));
}
...
function rarityEmoji(tier){
  return {legendary:'💎', epic:'🔵', rare:'🟢', common:'⚪', empty:'⬜', wrong:'❌'}[tier] || '⬜';
}
function rarityLabel(tier){
  return {legendary:'💎 Legendary (<1%)', epic:'🔵 Epic (<5%)', rare:'🟢 Rare (<15%)', common:'⚪ Common'}[tier] || '';
}
...
function buildShareGrid(rarityData){
  ...
  for(let r=0; r<n; r++){
    ...
    for(let c=0; c<n; c++){
      const i = r*n+c;
      const filled = G.gstate[i] !== null;
      if(!filled){ row += '⬜'; continue; }
      if(rarityData && rarityData[i]){
        const total = rarityData[i].total || 0;
        let pct = null;
        if(total > 0){
          const picks = rarityData[i].top || [];
          const myPick = G.filled_by[i];
          const myEntry = picks.find(p => p.name === myPick);
          if(myEntry) pct = (myEntry.count / total * 100);
        }
...
function toggleSolutions(){
  ...
  if(panel.style.display==='none'){
    ...
    btn.textContent='🔍 View All Answers';
  }
}
...
function renderSolutions(){
  ...
  grid.forEach((cell, i) => {
    ...
    if(rarityData && rarityData[i] && rarityData[i].total > 0){
      ...
      rarityHtml = pct !== null
        ? `<span class="rarity-chip rarity-${tier}" style="margin-left:6px;">${rarityEmoji(tier)} ${pct < 1 ? '<1' : Math.round(pct)}%</span>`
        : '';
      // Show top 3 picks
      const topPicks = rarityData[i].top.slice(0,3).map(p => {
        const pp = (p.count/total*100);
        return `<span style="font-size:.68rem;color:var(--txt3);margin-right:8px;">
          ${p.name} <span style="color:var(--txt3);">${pp < 1 ? '<1' : Math.round(pp)}%</span></span>`;
      }).join('');
...
function end(reason){
  ...
  // Build filled_by map to send to server for rarity tracking
  const filledByMap = {};
  ...
  .then(r=>r.json()).then(d=>{
    ...
    document.getElementById('ed').textContent = isQuit
      ...
      : 'Acc: '+acc+'% · ✅ '+G.correct+' · ❌ '+G.wrong+wcNote+skipNote+' · '+elapsed+'s';

    // ── Streak ──
    if(d.streak && d.streak.is_new){
      const sb = document.getElementById('streak-block');
      const sc = document.getElementById('streak-content');
      const ss = document.getElementById('streak-sub');
      sb.style.display = '';
      if(d.streak.broken){
        sc.innerHTML = `<span style="color:var(--txt2);">💔 Streak reset to 1</span>`;
        ss.textContent = `Your streak was broken. Best: ${d.streak.best} days`;
      } else {
        ...
        const nextMilestone = [7,30,100,365].find(m => m > d.streak.current);
        ss.textContent = nextMilestone
          ? `${nextMilestone - d.streak.current} days to ${milestones[nextMilestone]}`
          : `All-time best: ${d.streak.best} days`;
    ...
    // ── XP ──
    if(d.xp){
      const xb = document.getElementById('xp-block');
      const xc = document.getElementById('xp-content');
      const xs = document.getElementById('xp-sub');
      const xbar = document.getElementById('xp-bar');
      xb.style.display = '';
      xc.textContent = `+${d.xp.gained} XP  ·  Level ${d.xp.level}: ${d.xp.name}`;
      const [toNext, range] = d.xp.to_next !== undefined ? [d.xp.to_next, d.xp.range] : [0, 1];
      ...
      if(d.xp.leveled_up){
        setTimeout(()=>{
          document.getElementById('levelup-name').textContent = `Level ${d.xp.level}: ${d.xp.name}`;
    ...
    if(shareText && !isQuit){
      document.getElementById('share-grid-text').textContent = shareText;
      document.getElementById('share-block').style.display = '';
    }

    document.getElementById('emod').style.display='flex';
    // Show sign-in nudge for guests
    if(!d.new_rating && !d.streak){
...
<div class="container page">
  <div class="flex justify-between items-center mb-5 flex-wrap gap-3">
    ...
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      {% if current_user.is_authenticated %}
        <div id="streak-info" style="display:none;"></div>
      {% endif %}
  ...
  card.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
      <div class="stat-card">
        ...
        <div class="stat-value" style="color:var(--acc);">${flame} ${s.current||0}</div>
        <div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">days</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Best Streak</div>
        <div class="stat-value" style="color:var(--pur);">${s.best||0}</div>
        <div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">days</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">Level</div>
        <div class="stat-value" style="color:var(--blue);font-size:1.6rem;">${xp.level||1}</div>
        <div style="font-size:.7rem;color:var(--txt3);margin-top:3px;">${xp.name||'Rookie'}</div>
      </div>
    </div>
...
def play():
    ...
    # Require login for competitive modes only
...
@login_required
def api_streak():
    """Return current user's streak and XP data."""
...
def api_end_game():
    """
    Finalise a game and update ratings, streak, XP, and rarity data.
    ...
    filled_by  = data.get("filled_by", {})   # {cell_idx: player_name}
    result     = {"rating_change": 0}
    season     = get_current_season()

    # ── Guest guard for authenticated-only modes ──────────────────────────────
    ...
    # ── Return rarity data for all modes (daily only has meaningful data) ─────
    if gmode == "daily":
        try:
...
║  New v7     → Daily streaks + freeze mechanic            ║
║  New v7     → XP system (15 levels, Rookie → Immortal)  ║
║  New v7     → Rarity tracking for daily cells            ║
║  New v7     → Emoji share grid (WhatsApp/Twitter)        ║
║  New v7     → Web Audio cricket sound effects            ║
║  New v7     → Guest play (no login for solo)             ║
╚══════════════════════════════════════════════════════════╝
""")
